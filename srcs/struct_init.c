...
t_data	*struct_init_2(t_data *data)
{
	...
	data->dead_connections = (int *)malloc(sizeof(int) * data->nb_rooms);
	...
	if (!data->path || !data->tab || !data->rooms || !data->final_path || !data->dead_connections)
		clean_all(data, 1);
	while (++i < data->nb_rooms)
	{
		data->path[i] = (int *)ft_memalloc(sizeof(int) * 1000);
		data->final_path[i] = (int *)ft_memalloc(sizeof(int) * 1000);
		data->rooms[i].name = (char *)ft_memalloc(sizeof(char) * 1000);
